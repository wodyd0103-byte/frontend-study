...
9~10강부터는 DAY 스냅샷 대신 Vite + React 19 + TypeScript 프로젝트 하나(`React/kopo14-app`)에서 실습을 진행합니다. 라우터를 배운 뒤로는 App.tsx가 실습 목록 메뉴가 되어, 링크로 각 실습 화면을 오갈 수 있습니다.
...
| 10-3 | TodoAdd2 | **자율과제** — 삭제(filter) · 완료 토글(map+spread) · 완료 카운터 |
| 10-4 | TitleCounter | useEffect + 의존성 배열 (문서 제목 바꾸기) |
| 10-5 | Timer | setInterval + cleanup + prev 갱신 |
| 10-6 | UserCrud | fetch CRUD 4종 + re-fetch 패턴 |
| 10-7 | GuestbookCrud | **자율과제** — 인라인 편집 모드까지 구현 |
| 10-8 | UserDetail + main/App | BrowserRouter · Routes · Link · useParams |
| 10-9 | UserDetail2 | **자율과제** — 목록 → 상세 라우팅 + 로딩 처리 |

실행: 터미널 2개 — `yarn start`(json-server 3000) + `yarn dev`(Vite 5173)

## 🍽️ 서현역 맛집 앱 (React/kopo14-project)

10강을 끝낸 당일 오후에 만든 **React 종합 미니프로젝트**. 음식점 21곳·리뷰 74개를 json-server로 서빙하며 9·10강 내용을 한 앱에 모았습니다.

- 정렬 탭(평점순/리뷰순) · 찜 필터 + 항목별 찜(PATCH로 서버 저장)
- 카테고리 필터 페이지(`/category` 라우트) · 목록 컴포넌트 공용 재사용
- 상세 모달 — 리뷰 fetch · 오버레이 클릭/ESC 닫기 · `stopPropagation`으로 중첩 클릭 제어
- `types.ts`에 공통 타입·API_URL을 모아 중복 제거, 모든 props에 interface 지정

실행: `cd React/kopo14-project && yarn && yarn start` + `yarn dev`
